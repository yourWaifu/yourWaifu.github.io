...
The library now knows what version is being used. You can know easily at runtime or compile time check the branch, build number, version hash, and if there's any modifications were made to the library.

You can also check the version based on if some feature are available.
...
I finally graduated from High School and I'm now in my first year of college. This effects the library in that I now have less time to spend working on the library as college in my experience requires more study time to pass classes.
...
Right now, the optional cache can easily take up the most memory. Optimizations like not storing duplicates and and adding some limits to the cache are plans for I have making the cache use up less memory.